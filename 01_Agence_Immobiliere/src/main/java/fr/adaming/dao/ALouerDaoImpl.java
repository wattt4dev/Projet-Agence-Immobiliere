...
public class ALouerDaoImpl implements IAlouerDao {
	...
	@Override
	public List<Alouer> getAlouerByDepartement(String region) {
		Query query = em.createQuery("FROM Alouer al WHERE al.region = :pRegion");
		query.setParameter("pRegion", region);
		List<Alouer> alouerRegion =query.getResultList();
		return alouerRegion;
	}
